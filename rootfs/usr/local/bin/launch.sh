...
if [[ "$LOGROTATE" = "true"] && ["$GOA" != "true" ]]; then while true; do logrotate --verbose --state /data/etc/logrotate.status /etc/logrotate; sleep 25h; done; fi &
if [[ "$LOGROTATE" = "true"] && ["$GOA" = "true" ]]; then while true; do killall goaccess; sleep 5s; logrotate --verbose --state /data/etc/logrotate.status /etc/logrotate; sleep 25h; done; fi &
